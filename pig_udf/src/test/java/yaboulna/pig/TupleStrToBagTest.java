...
public class TupleStrToBagTest {
  ...
  public void testOkFormat() throws IOException {
    ...
    DataBag actual = target.exec(TupleFactory.getInstance().newTuple(
        "(unigram1, unigram2, unigram2)"));
...
// assertTrue(expected.contains((String)actualT.get(0)));
    assertEquals("(unigram1)", (String) actualT.get(0));
// }
    actualT = iter.next();
    assertEquals(1, actualT.size());
    assertEquals("(unigram2)", (String) actualT.get(0));
    actualT = iter.next();
    assertEquals(1, actualT.size());
    assertEquals("(unigram2)", (String) actualT.get(0));
