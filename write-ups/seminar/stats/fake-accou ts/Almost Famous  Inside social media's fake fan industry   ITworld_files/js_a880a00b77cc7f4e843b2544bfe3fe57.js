 ...
 * Behavior to add AJAX voting support to polls.
 */
Drupal.behaviors.ajaxPollResults = function(context) {
  $('form.ajax-poll:not(.ajax-poll-results-processed)', context).addClass('ajax-poll-results-processed').each(function() {
    // Find the form and poll wrapper items that will be affected.
    var $form = $(this);
    var $pollWrapper = $form.parents('.content, .poll').filter(':first');
		
    // Find all the settings for this form.
    var url = $form.find('input[name=ajax_results_url]').val();
    
		var $resultLink = $pollWrapper.parent().find('a.ajax-see-results');
		var resultsUrl = url;
		var backtoVotesUrl = url.replace('results','back');
		var $backLink = $('<a href="'+backtoVotesUrl+'">Back to choices</a>');
		
		var $showLoadedResultsLink = $('<a href="'+url+'">View Poll results</a>').click(function(event){
			event.preventDefault();
			$form.hide();
			$('#resultswrap').show();
			$(this).hide();
		...
		$backLink.click(function(event){
			event.preventDefault();
			$backLink.hide().after($showLoadedResultsLink);			
			$showLoadedResultsLink.show();
			$('#resultswrap').hide();
			$form.show();
		});
		...
		$resultLink.click(function(event){
			event.preventDefault();
			$(this).hide().before($loadingText);
			$.getJSON(url, function(data) {
				var $resultsOutput = $('<div id="resultswrap">' + data.output + '</div>');
				$form.hide();
			  $pollWrapper.append($resultsOutput);
				$loadingText.hide().before($backLink);
			});
		});
  });
};
 ...
 * debugging code.
 */
Drupal.theme.prototype.ajaxPollResultsError = function() {
  return '<div class="messages error">A parsing or network error has occurred.</div>';
};
...
return 0;var el=e.jquery!=undefined?e[0]:e;return $jc.intval($.css(el,p));},intval:function(v){v=parseInt(v);return isNaN(v)?0:v;}});})(jQuery);;
/*
 * jQuery UI 1.7.3
 *
			...
			for( method in rules ) {
				...
				try {
					...
					if(result == "same"){
						...
						errors[element.name] = $.isFunction(previous.message) ? previous.message(previous.old) : previous.old;
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			if ( previous.old !== value ) {
				...
				return "pending";
			} else if( previous.old === value && previous.valid === false){
				//FIX REMOTE MESSAGE TWICE ON BLUR
				//RETURN DE PREVIOUS OBJECT DIRECTLY
				return "same";
			}
...
$(document).ready(function(){
    ...
    $("#user-register").validate({
                            ...
                            var errors = this.numberOfInvalids();
                                if (errors) {
                                        var message = errors == 1
                                                ? 'Oops, there was a problem. Please correct the highlighted field below.'
                                                : 'Oops, there were some problems. Please correct the highlighted fields below.';
                                        $("div.reg-error span").html(message);
                                        $("div.reg-error").show();
                                } else {
                                        $("div.reg-error").hide();
                                }
                        ...
                        rules: {
                            ...
                            'field_state[value]' : {
                                required: "#edit-field-country-value[value=United States]"
                            }
                        },
...
Drupal.behaviors.userRegisterForm = function() {
  //shows state form only when country is US
  if ($('#user-register').size()) {
    if ($('#user-register #edit-field-country-value-wrapper select').val() != 'United States') {
      $('#user-register #edit-field-state-value-wrapper').hide();
    }

    setInterval(function() {
      if ($('#user-register #edit-field-country-value-wrapper select').val() == 'United States') {
        $('#user-register #edit-field-state-value-wrapper').show();
      }
      else {
        $('#user-register #edit-field-state-value-wrapper').hide();
      }
    ...
    // shows * for pseudo-required state field
    $('#user-register #edit-field-state-value-wrapper label[for=edit-field-state-value]')
     .append('<span class="form-required" title="' + Drupal.t('This field is required.') + '">*</span>');
  }
}


Drupal.behaviors.focusOnFirstItem = function() {
  //first we get our form and exit otherwise
  $form = $('#user-register');
  if (!$form.size()) {
    $form = $('#user-login');
    if (!$form.size()) {
  ...
  $form.find('.form-text:first').focus();
}

Drupal.behaviors.modalLink = function() {
  //convert nojs into ajax in links
  $('a.ctools-use-dialog').each(function() {
    $this = $(this);
    $this.attr('href', $this.attr('href').replace('nojs', 'ajax'));
  });
}

Drupal.behaviors.allowModalOverFlash = function() {
  $('param[name=wmode]').attr('value', 'transparent').parent().css({position: 'absolute', 'z-index': 10}).css({position: 'relative'});
};
...
function itw_ajax_friends_request(uid) {
    ...
    //            setTimeout('itw_ajax_followers_request_go()', 200);
    $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
    $('#tabset-tab-1').addClass('ui-tabs-hide');
    $('#tabset-tab-2').addClass('ui-tabs-hide');
    $('#tabset-tab-3').addClass('ui-tabs-hide');
    $('#tabset-tab-4').addClass('ui-tabs-hide');
     
    if($('#friends-container').is(":visible")){
    	$('#friends-container').load("/profile/content/friends/"+uid+"/main", {} ,function(){
        FB.XFBML.parse();
...
function itw_ajax_comments(value,tab) {
    ...
    // $('#profile-comments-container').html('<p><img src="/sites/all/themes/itworld/images/ajax-loader.gif" width="220" height="19" /></p>');
    //            setTimeout('itw_ajax_comments_go('+value+')', 200);
    $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
    $('#tabset-tab-1').addClass('ui-tabs-hide');
    $('#tabset-tab-2').addClass('ui-tabs-hide');
    ...
    $('#tabset-tab-4').addClass('ui-tabs-hide');
    if(tab == 'user'){
	 $('#tabset-tab-4').removeClass('ui-tabs-hide');
	 $('.tabset-tab-4').removeClass('ui-state-default');
     $('.tabset-tab-4').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
    }else{
     $('#tabset-tab-3').removeClass('ui-tabs-hide');
     $('.tabset-tab-3').removeClass('ui-state-default');
     $('.tabset-tab-3').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
    }
    $('#profile-answers').load("/profile/content/comments/"+value);
}

function itw_ajax_scorecomments(value,tab){
    // $('#profile-comments-container').html('<p><img src="/sites/all/themes/itworld/images/ajax-loader.gif" width="220" height="19" /></p>');
    //            setTimeout('itw_ajax_comments_go('+value+')', 200);
    $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
    ...
    if(tab == 'user'){
	 ...
	 $('.tabset-tab-4').removeClass('ui-state-default');
     $('.tabset-tab-4').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
     $('#tabset-tab-4').load("/profile/content/scorecomments/"+value);
    }else{
     $('#tabset-tab-3').removeClass('ui-tabs-hide');
     $('.tabset-tab-3').removeClass('ui-state-default');
     $('.tabset-tab-3').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
     $('#tabset-tab-3').load("/profile/content/scorecomments/"+value);
    }
    
}

function itw_ajax_qa(value,tab){
    // $('#profile-comments-container').html('<p><img src="/sites/all/themes/itworld/images/ajax-loader.gif" width="220" height="19" /></p>');
    //            setTimeout('itw_ajax_comments_go('+value+')', 200);
    $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
    ...
    if(tab == 'user'){
	 $('#tabset-tab-4').removeClass('ui-tabs-hide');
	 $('.tabset-tab-4').removeClass('ui-state-default');
	 // need to return the tab's original state because it's being altered here
	 $('.tabset-tab-4 span.tab').click(function() { 
		 itw_ajax_comments('newest,'+uid[1],tab);
     });
     //$('.tabset-tab-4').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
    }else{
     $('#tabset-tab-3').removeClass('ui-tabs-hide');
     $('.tabset-tab-3').removeClass('ui-state-default');
     // need to return the tab's original state because it's being altered here
     $('.tabset-tab-3 span.tab').click(function() { 
		 itw_ajax_comments('newest,'+uid[1],tab);
     });
     //$('.tabset-tab-3').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
    }
...
function itw_ajax_author(value, tab) {
	$('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
    $('#tabset-tab-1').addClass('ui-tabs-hide');
    $('#tabset-tab-2').addClass('ui-tabs-hide');
...
function itw_ajax_likes(value,tab){
    // $('#profile-comments-container').html('<p><img src="/sites/all/themes/itworld/images/ajax-loader.gif" width="220" height="19" /></p>');
    //            setTimeout('itw_ajax_comments_go('+value+')', 200);
    $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
    $('#tabs-tabset li').addClass('ui-state-default');
...
function itw_ajax_activity(value,tab){
    ...
    $('#tabset-tab-4').addClass('ui-tabs-hide');
	 ...
	 $('#tabset-tab-1').removeClass('ui-tabs-hide');
	 $('.tabset-tab-1').removeClass('ui-state-default');
     $('.tabset-tab-1').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
    
    
}

function itw_ajax_comments_go(value) {
    //var value;
    $('#profile-comments-container').load("/profile/content/friends/"+value+"/main");
}
function itw_ajax_page(user,page,order){
    //alert('page: '+page);
    $('#profile-comments-container').load("/profile/mycomments/"+user+"/"+order+"/"+page, function(){
			window.scrollTo(0, $('#profile-answers-sort').position().top);
		});
		
}
function itw_ajax_pageq(user,page){
    //alert('page: '+page);
    $('#profile-comments-container').load("/profile/myquestions/"+user+"/"+page);
}
function itw_ajax_pagea(user,page){
    //alert('page: '+page);
    $('#profile-comments-container').load("/profile/myanswers/"+user+"/"+page);
}

function itw_ajax_pageact(user,page){
    //alert('page: '+page);
    $('#profile-activity').load("/profile/friendsact/"+user+"/"+page);
}

function itw_ajax_pageactm(user,page){
    //alert('page: '+page);
    $('#profile-my-activity').load("/profile/myact/"+user+"/"+page);
}
        
function itw_readmore() {
    $('#bioshort').toggle('slow', function() {
	// Animation complete.
	});
    $('#biofull').toggle('slow', function() {
	// Animation complete.
	});
}

function itw_stealth(mode,uid) {
    //$.get('/profile/stealth/'+mode+'/'+uid);
    $.ajax({
	url: '/profile/stealth/'+mode+'/'+uid,
	success: function(data) {
	    $('#stealthon').toggleClass('selected');
	    $('#stealthoff').toggleClass('selected');
	    $('#hstealthon').toggleClass('selected');
	    $('#hstealthoff').toggleClass('selected');
...
$(document).ready(function(){
  $('#editpwdlink').click(function(){
	$('#changepasswrap').toggle();
	return false;
    });
  $('#changepwdclose').click(function(){
    $('#changepasswrap').hide();
    return false;
  });
  
  if ($('#unfollowconfirm').length > 0) {
	  $('#unfollowconfirm').dialog({
	    autoOpen: false
	  });
	  
	  $('#unfollowconfirmdone').dialog({
	    autoOpen: false
	  });
  }
  ...
  if (path[1] == 'profile' && path[2] == 'edit' && path[4] == 'alerts') {
	  $('#tabs-tabset li').removeClass('ui-tabs-selected ui-state-active active');
	  $('#tabset-tab-2').addClass('ui-tabs-hide');
	  $('#tabset-tab-1').removeClass('ui-tabs-hide');
	  $('.tabset-tab-1').addClass('ui-state-focus ui-tabs-selected ui-state-active active');
  }
  profile_country_state_visibility();
  profile_newsletters_country_state_visibility();
  
  $("label[for='edit-field-state-value']").append('<span title="This field is required." class="form-required">*</span>');
  $('#edit-field-country-value').change(function(){ 
	  profile_country_state_visibility();
  });

  $("label[for='edit-profile-field-state']").append('<span title="This field is required." class="form-required">*</span>');
  $('#edit-profile-field-country').change(function(){
	  profile_newsletters_country_state_visibility();
  });
  
  if (window.location.hash == '#questions') {
    $('a#profile-questions-asked').click();
  }
  
  if (window.location.hash == '#answers') {
	$('a#profile-questions-answered').click();
  }
});

function unfollowconfirm(username, ka_uid, userID){
  $('#follow_text').html("<p>Are you sure you want to stop following " + username + "?</p>");
  $('#followconf_text').html("You are no longer following " + username);
  $('#follow_ok').attr('rel',username);
  $('#follow_ok').attr('rel2',ka_uid);
  $('#follow_ok').attr('rel3',userID);
  $('#unfollowconfirm').dialog( "option", "width", 450);
  $('#unfollowconfirm').dialog( "option", "minHeight", 270);
  $('#unfollowconfirm').dialog('open');
  return false;
}

function follow_close(){
  $('#unfollowconfirm').dialog('close');
  return false;
}

function follow_done_close() {
  $('#unfollowconfirmdone').dialog('close');
  return false;
}

function reallyUnfollow() {
	var username = $('#follow_ok').attr('rel');
	$('#follow_text').html('<p>Unfollowing ' + username + '<br/></p><img style="margin-left:10px;" width="32" height="32" title="Loading" alt="Loading..." src="/sites/all/modules/contrib/ctools/images/throbber.gif">');
	$('#follow_ok').hide();
	$('#follow_cancel').hide();
	var ka_userid =  $('#follow_ok').attr('rel2');
	var userID = $('#follow_ok').attr('rel3');
	var url = '/kickapps/follow-friend/' + ka_userid + '/remove/' + username;
	jQuery.getJSON(url, null, function() {
		$('#unfollowconfirm').dialog('close');
		$('#follow_ok').show();
		$('#follow_cancel').show();
		var following = parseInt($('span#profile-following-count').html());
		if (following == 1) {
			$('#following-box').hide(1000);
			$('#following').hide(1000);
			$('span#profile-following-count').html(0);
		}
		else {
			$('li#following-user-' + userID).hide(1000);
			$('span#profile-following-count').html(following - 1);
		}
	});
}

function socdel(account,uid,atype){
	var answer = confirm('Are you sure you want to remove '+account+'?');
	if(answer == true){
		window.location="/profile/socdelete/"+atype+"/"+uid;
	}
}

function profile_country_state_visibility() {
	if ($('#edit-field-country-value').val() != 'United States') {
		$('#edit-field-state-value-wrapper').hide();
		$('#edit-field-state-value').val('');
	}
	else {
		$('#edit-field-state-value-wrapper').show();
	}
}

function profile_newsletters_country_state_visibility() {
	if ($('#edit-profile-field-country').val() != 'United States') {
		$('#edit-profile-field-state-wrapper').hide();
		$('#edit-profile-field-state').val('');
	}
	else {
		$('#edit-profile-field-state-wrapper').show();
	}
}

function profile_form_move_specialization_tags() {
  $('#edit-taxonomy-tags-17-wrapper, #edit-taxonomy-tags-17-autocomplete').insertBefore('#edit-field-bio-0-value-wrapper');
}
;
...
/// Not sure that this can be made to work inside a UI dialog box - CL
 // $(".lyris_tooltip :description").tooltip({
 //     events: {
 ...
 * action.
 */
Drupal.behaviors.itw_activity = function(context) {
  Drupal.itw_activity.widget.sharebar();
  Drupal.itw_activity.widget.fblike();
  Drupal.itw_activity.widget.linkedin();
  Drupal.itw_activity.widget.live();
};

/**
 * General functionality which ITW Activity module depends on.
 */
Drupal.itw_activity = {};
Drupal.itw_activity.widget = {};

/**
 * Invokes the hook_menu() from itw_activity.module.
 *
 * @param string op
 *    This operation should match one of the $info->hooks on the
 *    itw_activity.module.
 * @param object type
 *    An object that contains possible extra arguments to send to the hook_menu.
 */
Drupal.itw_activity.request = function(op, type) {
  // Global overrides
  if(! type.nid) {
    // The NID needs to be overriden since on full node pages, the var nid is
    // already set.
    type.nid = document.location.href.split('/')[4];
  }

  if (! type.alias) {
    type.alias = $('title').html();
  }

  $.ajax({
    url: '/itw_activity/ops/insert',
    dataType: 'json',
    async: true,
    data: {
      'nid': type.nid,
      'op': op,
      'alias': type.alias,
      'itw_op_machname': type.itw_op_machname,
      'itw_op_friendlyname': type.itw_op_friendlyname
    },
    success: function(data, status, xhr) {
      if(data.response.success) {
        // If data needs to be returned, manage it here. e.g. recording an entry to
        // watchdog through an Ajax request.
...
/**
 * Implementation of Activity Widget.
 * This is the function that can be generally called in order to record an
 * Activity.
 */
Drupal.itw_activity.widget.generic = function(op, type) {
  // var type = {'nid': 53248};
  // var op = 'ka_comment';
  // Drupal.itw_activity.widget.generic(op, type);
  Drupal.itw_activity.request(op, type);
}

/**
 * Implementation of Activity Widget Sharebar
 */
Drupal.itw_activity.widget.sharebar = function() {
  var sharebar_type = [
    {
      'e': 'st_plusone_vcount',
      'itw_op_machname': 'gplus',
      'itw_op_friendlyname': 'Google Plus'
    },
    {
      'e': 'st_linkedin_vcount',
      'itw_op_machname':'linkedin',
      'itw_op_friendlyname':'LinkedIN'
    },
    {
      'e': 'st_twitter_vcount',
      'itw_op_machname':'twitter',
      'itw_op_friendlyname':'Twitter'
    },
    {
      'e': 'st_reddit_vcount',
      'itw_op_machname':'reddit',
      'itw_op_friendlyname':'Reddit'
    },
    {
      'e': 'st_stumbleupon_vcount',
      'itw_op_machname':'stumbleuppon',
      'itw_op_friendlyname':'StumbleUpon'
    },
    {
      'e': 'st_email_vcount',
      'itw_op_machname':'email',
      'itw_op_friendlyname':'Email'
    }
  ];

  jQuery.each(sharebar_type, function(i, v) {
    var type = {
      itw_op_machname: v.itw_op_machname,
      itw_op_friendlyname: v.itw_op_friendlyname
    };

    jQuery('.' + v.e).bind('click', function (e) {
      Drupal.itw_activity.request('sharebar', v);
    });
  });
}

/**
 * Implementation of Activity Widget Facebook Like
 */
Drupal.itw_activity.widget.fblike = function() {
  jQuery(document).bind('fb:init', function (){
    FB.Event.subscribe('edge.create', function (href) {
      var alias = encodeURIComponent(href);
      var nid = unescape(href).split('/')[4];
      var type = {'nid': nid, 'alias': alias, 'itw_op_machname': 'fb_like', 'itw_op_friendlyname': 'Facebook'};
      Drupal.itw_activity.request('fb_like', type);
    });
  });
...
/**
 * Implementation of Activity Widget LinkedIN
 */
Drupal.itw_activity.widget.linkedin = function() {
  $('.st_linkedin_custom').click(function() {
    var alias = encodeURIComponent($(this).attr('st_url'));
    var nid = $(this).attr('st_url').split('/')[4];
    var type = {'nid': nid, 'alias': alias, 'itw_op_machname':'linkedin', 'itw_op_friendlyname':'LinkedIN'}
    Drupal.itw_activity.request('linkedin', type);
  });
}

/*
 * The following is a reimplementation of the itw live block that is more
 * dynamic and less susceptible to page load slowness.
 */

/**
 * Implementation of Activity Widget Live
 */
Drupal.itw_activity.widget.live = function() {
  var livecarousel;
  var timer;
  ...
  function mycarousel_initCallback(carousel, state) {
    ...
    jQuery('#itworldlive-next').bind('click', function() {
      stopAuto();
      carousel.next();
    ...
    jQuery('#itworldlive-prev').bind('click', function() {
      stopAuto();
      carousel.prev();
    ...
    timer = setTimeout(function() {
      jQuery.getJSON(
        '/views/ajax/?view_name=itw_live&view_display_id=default', 
        null, 
  ...
  function mycarousel_itemAddCallback(carousel, first, last, response) {
    var $items = jQuery(response.display);
    var total = 0;
    
    $items.each(function(i) {
      var jqElement = jQuery(this)[0];
      
      if (jqElement.nodeName != 'LI') {
        return true;
      }
      
      carousel.add(total + 1, jqElement.children[0]);
      carousel.get(total + 1).addClass(jQuery.attr(jqElement, 'class'));
      total += 1;
    });
    
    carousel.size(total);
    carousel.options.start = total;
  ...
  if (!jQuery('#activitycarousel').data('jcarousel')) {
    ...
    if (jQuery("#disqus_thread").length) {
      ...
      disqus_config = function() {
        ...
        this.callbacks.onNewComment = [function(comment) {
            DisqusWrapper.updateCommentsStatistics(comment);
        }];
      };
...
/**
 * Make a DISQUS wrapper to handle specific site actions.
 */
 var DisqusWrapper = {
    updateURL: '/update/comments/statistics',

     updateCommentsStatistics: function(comment){
     $.post(
             DisqusWrapper.updateURL,
             {
                identifier: Drupal.settings.disqus.identifier,
                comments: Object.keys(DISQUS.jsonData.posts).length,
                text: comment.text
             }
         );
     }
...
$.extend($.ui.dialog.overlay, {
	instances: [],
	maxZ: 0,
	//events: $.map('focus,mousedown,mouseup,keydown,keypress,click'.split(','),  JS - hacking to reenable scrollbars in chrome
    events: $.map('focus,keydown,keypress,click'.split(','),
...
Drupal.behaviors.answersFilterByBehavior = function(context) {
  $('#edit-filter-by', context).change(function() {
    ...
    $(location).attr('href', base_url + '?filter-by=' + $('#edit-filter-by', context).val());
  });
};

Drupal.behaviors.answersPromoFilterByBehavior = function(context) {
  $('#edit-filter-answers-by', context).change(function() {
    var url = $(location).attr('href');
    var base_url = url.split("?")[0];
    $(location).attr('href', base_url + '?filter-answers-by=' + $('#edit-filter-answers-by', context).val());
  });
};

$(document).ready(function() {
	$('span[class="up-current-score"]').each(function(){
		if($.trim($(this).html()) == '(0)') {
			$(this).hide();
		}
	});

  $askButton = $('a[name=AnswerHomepage:AskQuestion]')
    .add('a[name=Homepage:AskQuestion]')
    .add('#listlogin a');

  if ($askButton.size()) {
    $('#edit-title').add('#edit-body').change(function() {
      $askButton.each(function() {
        $this = $(this);
        href = $this.attr('href');
        indexOfParams = href.indexOf('&question_title');
        if (indexOfParams > 0) {
          href = href.substring(0, indexOfParams);
        }
        href = href +
                '&question_title=' + $('#edit-title').val() +
                '&question_body=' + $('#edit-body').val();
        $this.attr('href', href);
      });
    });
  }
	// Clear tag placeholder before submit
	$('form.question-node-form input.form-submit').click(function(){
		$input = $('input#edit-taxonomy-tags-11');
		if( $input.val() == 'Example, tag 1, tag 2, tag 3, tag 4' ){
			$input.val('');
		}
		$('form.question-node-form').submit();
	});
});
;
...
$(document).ready(function(){
    ...
    // Header Nav Dropdown IXD
    $('#blog-dd, #term-dd').hover(function(){
        $(this).prev().addClass('hover');
    }).bind('mouseleave', function(){
        $(this).prev().removeClass('hover');
    });
    // Secondary Nav Dropdown IXD
    $('.sl-has-dd').hover(function(){
        $(this).find('.sl-dd').show();
        $(this).addClass('hover');
    }).bind('mouseleave', function(){
        $(this).find('.sl-dd').hide();
        $(this).removeClass('hover');
    });
    
    // Leadgen Dropdown ixd
    $('li.lg-sort-dd').hover(function(){        
        $(this).addClass('hover');
    }).bind('mouseleave', function(){        
        $(this).removeClass('hover');
    });

		// IXD for answers dropdown
		$('#add-mt').hover(function(){
			 $(this).addClass('hover');
		}).bind('mouseleave', function(){
			 $(this).removeClass('hover');
		});

    // remove password strength check by drupal
    // has to be in this file to happen after user.js
    Drupal.behaviors.password = function(context) {
        return;
    }

    // Header dropdowns    
    $('#header-profile-dropdown li').click(function(event){
        event.stopPropagation();
    });
    $('li.dropdown-items a.count').click(function(){                
        $this = $(this).parent();
        if($this.hasClass('expand')){
            $this.toggleClass('expand');
        }else{
            $('.dropdown-items').removeClass('expand');        
            $this.toggleClass('expand');
        }
    ...
    $('li.dropdown-items a.closeDropdown').click(function(){
...
/*    $('div.profilebio').expander({
		...
		$('div.profilebio').truncate({max_length: 190, more: 'read more',   less: 'read less'});

    // hide ask a question tab on window < 970px
    $(window).resize(function() {
        $('#ask-tab').toggle($(window).width() > 1024);
    }).resize();
    ...
    // tooltips
    $('.cluetip-left-yellow').tooltip({
      showURL: false,
      top: -15,
      left: -10,
      opacity: 1,
      extraClass: "left-yellow", 
      positionLeft: true
    });

    $('.cluetip-right-yellow').tooltip({
      showURL: false,
      top: -15,
      right: -10,
      opacity: 1,
      extraClass: "right-yellow", 
      positionRight: true
    });
  ...
  showBody: " - ",
    ...
    $("#poll-view-voting").validate({
        showErrors: function(errorMap, errorList) {
          ...
          if (errors) {
            ...
            : 'At least one choice must be selected.';
            $("div.vote-error span").html(message);
            $("div.vote-error").show();
        } else 
            $("div.vote-error").hide();
        },
    ...
    //Validation for newsletter page
    if( $('#lyris-newsletter-listing-form').length > 0 ){
        jQuery.validator.addMethod('newsletters',function(value, element) {
            return $("#lyris-newsletter-listing-form :checkbox").filter(':checked').length >= 1;
        }, 'error');

        $("#lyris-newsletter-listing-form").validate({
            showErrors: function(errorMap, errorList) {
              var errors = this.numberOfInvalids();
              if (errors) {
                var message = errors == 1
                $("div#newsletter-error").show();
            } else
            ...
            submitHandler: function(form) {
                var selectionstring = '';
                $checked = $("#lyris-newsletter-listing-form input:checked");
                $numchecked = ($checked.length) - 1;
                $checked.each(function(index){
                   $name = $(this).attr('name');
                   $newname = $name.replace('field_', '');
                   if( index < $numchecked ){
                     selectionstring += $newname + '^';
                   }else{
                     selectionstring += $newname;
                   }
                });
                
                $('#trigger-news-reg').attr('href', '/newsletters/signup/'+selectionstring);
                $('#trigger-news-reg').click();
            }
        });
        $("#lyris-newsletter-listing-form :checkbox").rules("add", 'newsletters');
        
    }    

    // hide trending if ad ticker is populated
    
    if( $('#block-doubleclick_ads-ticker img[src="http://s0.2mdn.net/viewad/817-grey.gif"]').length == 0 ){
        $('#block-views-ITWorldTrending-block_1').html('');
    }else{
        $('#block-views-ITWorldTrending-block_1').css({'visibility':'visible', 'height':'auto'});
    }

		pageCheck = setInterval(checkPageVis, 500);

}); // End docReady

var pageCheck = '';
function checkPageVis(){
	if($('#page').is(':visible')){
		jQuery('#viewscarousel-editor_picks-block_1').jcarousel();
...
/*
 *
 * Activity Module Ajax Functionality
 * Frank Cefalu
...
$(document).ready(function(){
    /*
   * Start Event Bind Listeners
   * Articles Share Bar Functionality
   *
   */
    $('.socialArticleBar').bind('click',function() {

        var loggedIn  = Drupal.settings.fbconnect.user.uid * 1;
        var data = {
            "parentCode":this.className.replace(" socialArticleBar"),
            "parentName":this.displayText
            }
    ...
    if($('#sharebar').length != 0){
        ...
        $('.socialArticleBarLoader').fadeOut();
    }
});

// Built the following to handle any AJAX Requests for the Activity Module.
// This will be expanded, and continue to be for any ajax requests necessary for the site.

function publish_linkedin(url, publishText) {
	window.twttr=window.twttr||{};
	var D=550,A=450,C=screen.height,B=screen.width,H=Math.round((B/2)-(D/2)),G=0,F=document,E;
	if(C>A){G=Math.round((C/2)-(A/2))}
	window.twttr.shareWin=window.open("http://www.linkedin.com/shareArticle?mini=true&url=" + encodeURIComponent(url) + "&title=" + escape(publishText) ,'','left='+H+',top='+G+',width='+D+',height='+A+',personalbar=0,toolbar=0,scrollbars=1,resizable=1');
}

function postActivityAjax(type,data) {
    ...
    $.ajax({
        type: "POST",
        url: "/activity/submit/"+type,
        data: render,
        // May need this later For Logged in Users
...
function itw_validate($formid){
  $itsvalid = '';
  switch($formid) {
    case 'user-register' :
      /*jQuery.validator.addMethod('areasofinterest',function(value, element) {
        return $("input[name^='field_areas_of_interest']").filter(':checked').length >= 1;
      }, 'error');*/
      $("#user-register").validate({
        showErrors: function(errorMap, errorList) {
          var errors = this.numberOfInvalids();
        ...
        errorPlacement: function(error, element) {
          if( element.hasClass('form-checkbox')){
            ...
            $newSelect = element.next();
            error.insertAfter( $newSelect );
          }else{
            error.insertAfter( element);
          }
        },
      ...
      //$("input[name^='field_areas_of_interest']").rules("add", 'areasofinterest');
      break;
    case 'user-login' :
      $("#user-login").validate({
        showErrors: function(errorMap, errorList) {
...
/* 
 * Generic FB login function - moved from itw_login
 */
function Login() {
   if (!FB.getAuthResponse()) {
        FB.login(function(response) {
          $("#fbconnect-autoconnect-form").submit();
            document.getElementById('fb_bt').innerHTML = 'Logout';
            if (response.session) {
                ...
                document.getElementById('fb_bt').innerHTML = 'Login with Facebook';
            }
...
//quickreg facebook login and auto-comment-submit
function QR_Login() {
   if (!FB.getAuthResponse()) {
        FB.login(function(response) {
            if (response.session) {
                FB.api('/me', function(response) {
					// call the ajax to reg the user
					jQuery.ajax({
					type: "GET",
					url: "/soc/fbquickreg",
					success: function(result) {
					    var reg = JSON.parse(result);
					    if(reg.status == "success"){ // Reg is ok
							if (jQuery('#edit-comment-body').val() == "" || jQuery('#edit-comment-body').val() == "Type your comment here") {
							    window.location.reload();
						    } else {
						    	ka_do_comment();
						    }
						    ...
						    alert("We're sorry, an error occurred posting your comment.  Please try with a different account, or try again later.");
							return false;
					    }
					}, 
					error: function(jqXHR, textStatus, errorThrown) {
						alert("Sorry, an error occurred posting your comment.  Please try with a different account, or try again later.");
					}
				    }); // end ajax wrap
                });
            }
            else {
      			alert("Sorry, there was an error signing you in with Facebook.");
            }
...
/* truncate */
(function(b){var i=true;var d=0;b.fn.truncate=function(j){var k=b.extend({},b.fn.truncate.defaults,j);b(this).each(function(){var n=b.trim(f(b(this).text())).length;if(n<=k.max_length){return}d=k.max_length+k.more.length;var m=a(this);var l=b(this).hide();m.insertAfter(l);e(m).append('<a href="#">... '+k.more+"</a>");h(l).append('<br /><a href="#">'+k.less+"</a>");m.find("a:last").click(function(){m.hide();l.show();return false});l.find("a:last").click(function(){m.show();l.hide();return false})})};b.fn.truncate.defaults={max_length:100,more:"&hellip; read more",less:"read less"};function a(j){return(j.nodeType==3)?c(j):g(j)}function g(j){var j=b(j);var l=j.clone().empty();var k;j.contents().each(function(){if(d==0){return false}k=a(this);if(k){l.append(k)}});return l}function c(j){var k=f(j.data);if(i){k=k.replace(/^ /,"")}i=!!k.match(/ $/);var k=k.slice(0,d);k=b("<div>").text(k).html();d=d-k.length;return k}function f(j){return j.replace(/\s+/g," ")}function e(l){var j=b(l);var k=j.children(":last");if(!k){return l}var m=k.css("display");if(!m||m=="inline"){return j}return e(k)}function h(l){var j=b(l);var k=j.children(":last");if(k&&k.is("p")){return k}return l}})(jQuery);
...
/**
 * @author Diego Roldan
 * This had to be done through Javascript since the contributing module Primary
 * Term could add the dropdown after a module would try to modify its form
 * elements.
 */
$(document).ready(function() {
  if(typeof Drupal.settings.itw_articles != 'undefined') {
    if(Drupal.settings.itw_articles.article_node_form) {
      $('#edit-primaryterm').prepend(
        $('<option></option>').val('none').html(' - none - ')
      );
      $("#edit-primaryterm option[value='none']").attr('selected', 'selected');
    }
  }
});
 ...
 *   http://www.gnu.org/licenses/gpl.html
 */
eval(function(p,a,c,k,e,d){e=function(c){return(c<a?'':e(parseInt(c/a)))+((c=c%a)>35?String.fromCharCode(c+29):c.toString(36))};if(!''.replace(/^/,String)){while(c--){d[e(c)]=k[c]||e(c)}k=[function(e){return d[e]}];e=function(){return'\\w+'};c=1};while(c--){if(k[c]){p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c])}}return p}(';(9($){e 2={},a,k,A,B=$.2v.1O&&/1M\\s(5\\.5|6\\.)/.1Y(1S.2h),I=Z;$.f={u:Z,1j:{Y:1T,r:Z,19:1h,T:"",j:15,d:15,K:"f"},1R:9(){$.f.u=!$.f.u}};$.P.1i({f:9(4){4=$.1i({},$.f.1j,4);1q(4);b 3.E(9(){$.1d(3,"f",4);3.11=2.7.l("1g");3.X=3.k;$(3).1Q("k");3.1V=""}).1W(1r).23(n).24(n)},N:B?9(){b 3.E(9(){e J=$(3).l(\'16\');8(J.22(/^p\\(["\']?(.*\\.21)["\']?\\)$/i)){J=1X.$1;$(3).l({\'16\':\'1N\',\'1f\':"25:1K.1G.1F(1H=1h, 1L=1I, 1l=\'"+J+"\')"}).E(9(){e C=$(3).l(\'C\');8(C!=\'1J\'&&C!=\'1e\')$(3).l(\'C\',\'1e\')})}})}:9(){b 3},1o:B?9(){b 3.E(9(){$(3).l({\'1f\':\'\',16:\'\'})})}:9(){b 3},1c:9(){b 3.E(9(){$(3)[$(3).H()?"m":"n"]()})},p:9(){b 3.1k(\'1Z\')||3.1k(\'1l\')}});9 1q(4){8(2.7)b;2.7=$(\'<g K="\'+4.K+\'"><g K="2q-2p">-</g><R></R><g 1p="c"></g><g 1p="p"></g></g>\').2o(M.c).n();8($.P.Q)2.7.Q();2.k=$(\'R\',2.7);2.c=$(\'g.c\',2.7);2.p=$(\'g.p\',2.7)}9 4(1n){b $.1d(1n,"f")}9 1C(o){8(4(3).Y)A=2s(m,4(3).Y);q m();I=!!4(3).I;$(M.c).2x(\'18\',z);z(o)}9 1r(){8($.f.u||3==a||(!3.X&&!4(3).V))b;a=3;k=3.X;8(4(3).V){2.k.n();e G=4(3).V.2u(3);8(G.2m||G.2l){2.c.1b().12(G)}q{2.c.H(G)}2.c.m()}q 8(4(3).1s){e 10=k.2c(4(3).1s);2.k.H(10.2a()).m();2.c.1b();27(e i=0,17;(17=10[i]);i++){8(i>0)2.c.12("<2e/>");2.c.12(17)}2.c.1c()}q{2.k.H(k).m();2.c.n()}8(4(3).19&&$(3).p())2.p.H($(3).p().2g(\'2r://\',\'\')).m();q 2.p.n();2.7.13(4(3).T);8(4(3).N)2.7.N();1C.2i(3,2k)}9 m(){A=14;8((!B||!$.P.Q)&&4(a).r){8(2.7.O(":1m"))2.7.W().m().S(4(a).r,a.11);q 2.7.O(\':1x\')?2.7.S(4(a).r,a.11):2.7.2j(4(a).r)}q{2.7.m()}z()}9 z(o){8($.f.u)b;8(o&&o.2d.28=="29"){b}8(!I&&2.7.O(":1x")){$(M.c).1z(\'18\',z)}8(a==14){$(M.c).1z(\'18\',z);b}2.7.U("t-w").U("t-1y");e d=2.7[0].1v;e j=2.7[0].1A;8(o){d=o.2w+4(a).d;j=o.2y+4(a).j;e w=\'1w\';8(4(a).2t){w=$(F).1u()-d;d=\'1w\'}2.7.l({d:d,w:w,j:j})}e v=t(),h=2.7[0];8(v.x+v.1a<h.1v+h.1t){d-=h.1t+20+4(a).d;2.7.l({d:d+\'1E\'}).13("t-w")}8(v.y+v.1D<h.1A+h.1B){j-=h.1B+20+4(a).j;2.7.l({j:j+\'1E\'}).13("t-1y")}}9 t(){b{x:$(F).1P(),y:$(F).1U(),1a:$(F).1u(),1D:$(F).26()}}9 n(o){8($.f.u)b;8(A)2b(A);a=14;e D=4(3);9 L(){2.7.U(D.T).n().l("1g","")}8((!B||!$.P.Q)&&D.r){8(2.7.O(\':1m\'))2.7.W().S(D.r,0,L);q 2.7.W().2n(D.r,L)}q L();8(4(3).N)2.7.1o()}})(2f);',62,159,'||helper|this|settings|||parent|if|function|current|return|body|left|var|tooltip|div|||top|title|css|show|hide|event|url|else|fade||viewport|blocked||right|||update|tID|IE|position|tsettings|each|window|bodyContent|html|track|image|id|complete|document|fixPNG|is|fn|bgiframe|h3|fadeTo|extraClass|removeClass|bodyHandler|stop|tooltipText|delay|false|parts|tOpacity|append|addClass|null||backgroundImage|part|mousemove|showURL|cx|empty|hideWhenEmpty|data|relative|filter|opacity|true|extend|defaults|attr|src|animated|element|unfixPNG|class|createHelper|save|showBody|offsetWidth|width|offsetLeft|auto|visible|bottom|unbind|offsetTop|offsetHeight|handle|cy|px|AlphaImageLoader|Microsoft|enabled|crop|absolute|DXImageTransform|sizingMethod|MSIE|none|msie|scrollLeft|removeAttr|block|navigator|200|scrollTop|alt|mouseover|RegExp|test|href||png|match|mouseout|click|progid|height|for|tagName|OPTION|shift|clearTimeout|split|target|br|jQuery|replace|userAgent|apply|fadeIn|arguments|jquery|nodeType|fadeOut|appendTo|arrow|tt|http|setTimeout|positionLeft|call|browser|pageX|bind|pageY'.split('|'),0,{}))

/**
 * @author Roger Soper
 * Adding a s.prop for quick read link
 */
function quickread_omniture() {
    s.events="event24";
}

jQuery(document).ready(function() {

	if (jQuery('#edit-searchstring').length > 0) {
	    jQuery('#edit-searchstring').keydown(function(e){
	        if (e.keyCode == 13) {
	...
	if (jQuery('#edit-searchstring-1').length > 0) {
	    jQuery('#edit-searchstring-1').keydown(function(e){
	        if (e.keyCode == 13) {
	            $(this).parents('form').submit();
	            return false;
	        }
	...
	if (jQuery('#edit-searchstring-2').length > 0) {
	    jQuery('#edit-searchstring-2').keydown(function(e){
	        if (e.keyCode == 13) {
	            $(this).parents('form').submit();
	            return false;
	        }
...
(function(jQuery){jQuery.getQueryParam=function(param){var pairs=location.search.substring(1).split('&');for(var i=0;i<pairs.length;i++){var params=pairs[i].split('=');if(params[0]==param){return params[1]||'';}}return undefined;};})(jQuery);

// OMNITURE
function omniture_prop8_modules() {
	jQuery('#itworldlive a').each(function() {
		jQuery(this).attr("name", "&lpos=Activity");
	});
	jQuery('.viewscarousel a').each(function() {
		jQuery(this).attr("name", "&lpos=Carousel");
	});
    jQuery('#most-articles a').each(function() {
		jQuery(this).attr("name", "&lpos=Most");
	});
	jQuery('#sh_job_widget a').each(function() {
		jQuery(this).attr("name", "&lpos=ITJobs");
	});
	jQuery('#sponsored-links a').each(function() {
		jQuery(this).attr("name", "&lpos=SponsoredLinks");
	});
	jQuery('.view-white-papers-and-webcast a').each(function() {
		jQuery(this).attr("name", "&lpos=Whitepaper");
	});	
	jQuery('#icu-assets a').each(function() {
		jQuery(this).attr("name", "&lpos=downloads");
	});	
	jQuery('#sharebar a').each(function() {
		jQuery(this).attr("name", "&lpos=Article:toolbar");
	});	
	jQuery('.answers a').each(function() {
		jQuery(this).attr("name", "&lpos=AnswersModule");
	});		
	jQuery('#article-toolbar a').each(function() {
		jQuery(this).attr("name", "&lpos=Article:BottomToolbar");
	});	
	jQuery('#lyris-small a').each(function() {
		jQuery(this).attr("name", "&lpos=Homepage:Newsletter");
	});	
	jQuery('.blog-index a').each(function() {
		jQuery(this).attr("name", "&lpos=Homepage:FeaturedBlog");
	});	
	jQuery('.pane-itw-articles-frontpage-featured-article-1col a').each(function() {
		jQuery(this).attr("name", "&lpos=Homepage:TopStory");
	});	
	jQuery('.pane-itw-blocks-itw-most-blogs a').each(function() {
		jQuery(this).attr("name", "&lpos=Blog:Most");
	});	
	jQuery('#blog-carousel a').each(function() {
		jQuery(this).attr("name", "&lpos=Blog:Carousel");
	});	
	jQuery('#blog-carousel .jcarousel-next-horizontal a').each(function() {
		jQuery(this).attr("name", "&lpos=Blog:PrevNext");
	});
	jQuery('#blog-carousel .jcarousel-prev-horizontal a').each(function() {
		jQuery(this).attr("name", "&lpos=Blog:PrevNext");
	});	
	jQuery('#ask-tab a').each(function() {
		jQuery(this).attr("name", "&lpos=QuestionTab");
	});	
	jQuery('.pane-itworldtrending a').each(function() {
		jQuery(this).attr("name", "&lpos=Trending");
	});	
	jQuery('#questions-tabbed-container a').each(function() {
		jQuery(this).attr("name", "&lpos=LatestQuestions");
	});	
	jQuery('.view-most-active-members a').each(function() {
		jQuery(this).attr("name", "&lpos=HighestRatedMembers");
	});	
	jQuery('#evergreen-tabbed-container a').each(function() {
		jQuery(this).attr("name", "&lpos=Answers:EvergreenNews");
	});	
	jQuery('#nice-menu-3 a').each(function() {
		jQuery(this).attr("name", "&lpos=Nav:Answers");
	});	
	jQuery('.ka_comments a').each(function() {
		jQuery(this).attr("name", "&lpos=Comments");
	});	
	jQuery('.si-topstory a').each(function() {
		jQuery(this).attr("name", "&lpos=Index:TopStory");
	});	
	jQuery('#header-signed-in a').each(function() {
		jQuery(this).attr("name", "&lpos=Profile:Bar");
	});	
	jQuery('.followers-box a').each(function() {
		jQuery(this).attr("name", "&lpos=Profile:Followers");
	});	
	jQuery('.following-box a').each(function() {
		jQuery(this).attr("name", "&lpos=Profile:Following");
	});	
	jQuery('#friends-container a').each(function() {
		jQuery(this).attr("name", "&lpos=Profile:Friends");
	});	
	jQuery('#left-col #wide-wrapper #main-wrapper .view-id-white_papers_and_webcast a').each(function() {
		jQuery(this).attr("name", "&lpos=Homepage:Whitepaper_Webcast");
	});	
	jQuery('#bigwrapper #left-col .yui-u .view-id-white_papers_and_webcast a').each(function() {
		jQuery(this).attr("name", "&lpos=Article:Whitepaper_Webcast");
	});	
		
	jQuery('#itwregisterbutton').click(function() {
	    s.events='ProdView';
        s.t();
	    s.events='Category;Product';
        s.t();

	});
	jQuery('.san').click(function() {
	    s.events='ProdView';
        s.t();
	});
	

	jQuery('.quickread').children().click(function() {
	    s.events='event24';
        s.t();
	});
    jQuery('.quickread').click(function() {
	    s.events='event24';
        s.t();
	});
}

function omniture_sharebar_modules() {
		jQuery('.st_digg_vcount').children().click(function() {
		    s.events='event27,event30';
	        s.t();
	        OPG.Eloqua.record_social_event();
	//        console.log("omniture tracked digg");
		});
		jQuery('.st_twitter_vcount').children().click(function() {
		    s.events='event26,event30';
	        s.t();
	        OPG.Eloqua.record_social_event();
	        
	  //      console.log("omniture tracked twitter");
		});
		jQuery('.st_email_vcount').children().click(function() {
		    s.events='event29,event30';
	        s.t();
	        OPG.Eloqua.record_social_event();
	        
	    //    console.log("omniture tracked email");
		});
		jQuery('.st_facebook_vcount').children().click(function() {
		    s.events='event28,event30';
	        s.t();
	        OPG.Eloqua.record_social_event();
	        
	      //  console.log("omniture tracked facebook");
		});
		jQuery('.st_sharethis_vcount').children().click(function() {
		    s.events='event30';
	        s.t();
	        OPG.Eloqua.record_social_event();
	        
	       // console.log("omniture tracked sharethis");
		});
		
		jQuery('.st_linkedin_custom').children().click(function() {
		    OPG.Eloqua.record_social_event();
		});
        
        jQuery('.br-tw').children().click(function() {
		    OPG.Eloqua.record_social_event();
		});
}
jQuery(document).ready(function() {
	setTimeout("omniture_prop8_modules();", 3000);
	
	setTimeout("omniture_sharebar_modules();", 5000);
});

function loadProfileClearActive() {
	jQuery('#profile_act_all').removeClass("current");
	jQuery('#profile_act_cit').removeClass("current");
	jQuery('#profile_act_not').removeClass("current");
}

function loadProfileActivityAll() {
    loadProfileClearActive();
	jQuery('#profile-my-activity').load("/profile/myactivity/all", null, function() {
		jQuery('#profile_act_all').addClass("current");
	});
}

function loadProfileActivityCit() {
	loadProfileClearActive();
	jQuery('#profile-my-activity').load("/profile/myactivity/cit", null, function() {
		jQuery('#profile_act_cit').addClass("current");
	});
}

function loadProfileActivityNot() {
	loadProfileClearActive();
	jQuery('#profile-my-activity').load("/profile/myactivity/not", null, function() {
		jQuery('#profile_act_not').addClass("current");
	});
...
jQuery(document).ready(function() {
	$('#dropdown-saying a').click(function() { 	
		jQuery.ajax({
		  url: "/itw-not/saying-update",
		  success: function($response) { }
		});
	});
});
...
jQuery(document).ready(function() {
	if (window.location.href.indexOf('/sitemap') != -1 ) {
		jQuery('.leaf a').each(function() {
			if (jQuery(this).attr("href").indexOf('itwnla_topnav') != -1) {
				var src = jQuery(this).attr("href");
				src = src.replace(/topnav/, "sitemap");
...
jQuery(document).ready(function() {
	// t = tab on profile page
	// st = sub tab of tab on profile page
	if (jQuery.getQueryParam("t") == "ma") {
		jQuery('a[href=#tabset-tab-1]').click();
		if (jQuery.getQueryParam("st") == "not") {
			loadProfileActivityNot();
		} else if (jQuery.getQueryParam("st") == "cit") {
			loadProfileActivityCit();
		} 
	} else if (jQuery.getQueryParam("t") == "mfa") {
		jQuery('a[href=#tabset-tab-2]').click();
	}
  // fix for IDGITWORLD-127
  else if (jQuery.getQueryParam('t') == 'mc') {
		jQuery('a[href=#tabset-tab-4]').click();
  }
});
...
function inlineFieldLabel (label, inputid, form)
{
  ...
  var form = $(form);      // your text input field
...
if ((browser=="Microsoft Internet Explorer") && (brow_ver < 7)) //By default the min. IE ver is set to 7. Change as desired.
	{
...
$(document).ready(function(){
	...
	// Show Blog Carousel after all blogs are loaded
	$("#viewscarousel-editor_picks-default").css("visibility","visible");
}); ;
