...
which has to be filtered out by mining algorithms.
...
Following are the main problems we address in this work.
After addressing them, the frequent itemset mining results become
...
shows that the hour by hour synopses of two different days
actually reflects what was happening on those days.
The synopses generated by a state of the art algorithm 
...
is available for download\footnote{blinded}. 
The evaluation is only empirical because our work
results in a synopsis that is made up of sets of items,
...
inhibiting the construction of a gold standard needed for evaluation
...
Zhao et al.~\cite{Zhao:2011:TKE:2002472.2002521} build summaries based on an adaptation of Latent Dirichlet Allocation (LDA) for Twitter.
Parikh and Karlapalem~\cite{parikh2013events} extract topics from Tweets using a simplified state machine for burst detection.
