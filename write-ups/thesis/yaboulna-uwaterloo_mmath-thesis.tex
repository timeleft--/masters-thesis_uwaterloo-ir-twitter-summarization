...
The user might share an original status message about the 
topic, or \emph{re-share} content posted by other users.
...
While an update operation is not supported in the model of frequent itemsets mining, 
...
In case of retweeting people actually start from the original tweet and modify it. 
...
and strongly closed itemsets. 
In the mining results of hour long epochs from the 
first two weeks of November 2012, 
the size of the difference is 1516.59 on average. 2,439.17
Itemsets in the difference are closed itemsets that are not \emph{distinct},
since a \emph{distinct} itemset is always included in a \emph{strongly closed itemset} 
even if it is the only one it contains.
The difference is made up mainly of 2-itemsets, 
and all itemsets in it are either frequent N-grams after flattening (do not have subsets)
or they do not satisfy the \emph{distinct} confidence requirements.
%belong to a particular fine grained topic (do not imply any superset with high confidence).
%and they are mainly language constructs.
If itemsets are restricted to those containing a topical keyword 
before taking the difference, its size decreases dramatically.
The difference becomes 7.2 itemsets on average
if the itemsets from the first two weeks of November 2012
were restricted to ones containing `obama' or `romney' before taking the difference.
In this case, an epoch is included in the average only if the keywords occur in it.
%were restricted to the following keywords before taking the difference:
%\texttt{obama, romney} %, israel, hamas, syria, bieber, gomez}. 
%The difference is mainly because of itemsets which are frequent N-grams,
%like \{president, barack, obama\} and \{mitt, romney\} for example. 
%Those itemsets are not \emph{distinct}, 
%because they are always talked about.
%Other 
The hour of the 2012 US elections victory announcement happens
to be the hour with the most number of itemsets in the difference, 
%where 90 itemsets including one of the keywords above 
because of the large number of posts about `obama' and `romney'.
%were not included in the final selection.
%and most of the other hours have no itemsets in the difference (
%However, all of the 90 itemsets were included in supersets.
%Below is 
%However, all of the 90 itemsets had an alternate 
%Table \ref{table:interestingCover} shows some examples of the most interesting itemsets
%in the difference from that hour, 
%along with supersets in which they were included.
%\begin{table}[h]
%\centering
%\begin{tabular}{|c|c|}
%\hline
%Filtered Itemset & Covering Alternative
%\\\hline
%\end{tabular}
%\label{table:interestingCover}
%\caption{Filtered out interesting itemsets and their covering alternatives in the selection}
%\end{table} 
However, inspecting the itemsets in the difference
shows that most of them are not interesting.
Furthermore, interesting ones are covered in the selection,
either by a subset or superset or by an alternative that had enough confidence.
Figure \ref{fig:closedNDistinct} is a listing of the itemsets in the difference of that hour. 
%Below each interesting itemset,  
%the covering itemset from the selection 
%is also shown
%for interesting itemsets.
Along with each interesting itemset, 
the covering itemset that is included 
in the selection is also shown.
...
The average number of positions  between an itemset  %backwards
and an itemset with which it could potentially be clustered 
is 1973.11.
%(member of the set C formed at line 2 of algorithm \ref{algo:alliance}) 
However, the standard deviation is 1443.64, 
indicating that the clustering candidates are 
found either %at positions that ar
