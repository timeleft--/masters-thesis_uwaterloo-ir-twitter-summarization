...
we then explain the algorithm on which we build
...
is a simple method to flatten the Zipfean distribution
...
We propose a condition for selecting itemsets,
a clustering scheme for grouping together itemsets
according to their topic,
and a ranking formula that is suitable
for ranking clusters according to the 
...
1& \{barack\} (4) & mitt (2), obama (3), romney (2) & {\small Items in $T_{\{`barack'\}}$ are counted. No items should be added to the itemset.}\\\hline %with `barack'
...
\Fn{LCM(s: Current itemset,   $w_{sh}$: Suffix head, \\ \hspace*{1.2in}$T_s$: Transactions (tweets) containing s)}{
	...
	\ForEach{$v \succ w_{sh}$ and $v \notin $ suffix}{
		\If{frequency[$v$] $\geq support$}{%\alpha * |E|$}{%\frac{|E|}{E.span}
			$T \gets T_s \cap T_{\{v\}}$	\tcp*{Results of query $s$ AND $v$}
...
\caption{Average of token frequency deciles at different times of day and maximum N} %-gram length
...
This will also filter out any itemsets %remaining language constructs, because they
...
$s_d$, and another \emph{distinct} itemset, $s_c$, % YES, not any closed itemset
...
The number of \emph{strongly closed itemsets} from an hour long epoch is 139.1 itemsets on average, 
%which is a very big reduction 
which is under 0.25\% of the number of closed itemsets returned by LCM 
...
Novelty is measured compared to %between the mined epoch
...
$\kappa$ is only 139.1, which is about 5.7\% of the number before filtering and clustering. %of closed itemsets.
...
we investigate the synopses of hours selected by the aid of Google Trends\footnote{http://www.google.ca/trends},
...
In the evening, events about the elections keep occupying the top 3 positions.
...
& \em The electoral college &  Republicans complaining about the voting system  & - \\ \cline{2 - 4}
& \em President of the unites states & Barack Obama is the 44th president of the USA   & - \\ \cline{2-4} %(uh oh!)
& \em who voted for & People turning against each other & - \\ \cline{2-4} % 
& \em once you go black you never go [back] & A popular cultural reference & - \\ \cline{2-4} %(uh oh!)
...
& \em same sex marriage	& Some states legalized it & -  \\ \cline{2-4}
& \em for recreational use & Colorado legalized Marijuana for recreational use   & - \\ \cline{2-4} %(uh oh!)
& \em acceptance speech & People anticipating the speech & - \\ \hline % 

\multirow{3}{*}{\texttt{00:30} }
& \em Delivered, sealed, signed & Stevie Wonder's song used in Obama's campaigns & -  \\ \cline{2-4}
& \em The best is yet to come & Quote from Obama's speech  & - \\ \cline{2-4} %(uh oh!)
& \em with the flag in her [hair]	&  During the speech, attention on social media goes to a woman appearing behind Obama on TV! & - \\ \hline %  \cline{2-3}

\multirow{3}{*}{\texttt{01:00} }
& \em Four more years & Barack Obama's tweet; the most retweeted in 2012\footnote{http://ca.news.yahoo.com/blogs/ticket/obama-wins-twitter-election-most-rt-d-tweet-174452713--election.html} & 6 \\  \cline{2-4}
& \em @paulmyers47: spots & Pyramidal marketing; people retweet to make money & 26 \\  \cline{2-4}
...
\emph{@realDonaldTrump: This election is a total sham and a travesty. We are not a democracy!} & 1 & 2  \\\cline{2-4} % 266035509162303492
...
& 4 & 48
\\ \cline{2-4}
& \emph{House of Representatives shouldn't give anything to Obama unless he terminates Obamacare.} 
& 6 & 15 \\\cline{2-4}
% 266040877552656385
...
\end{tabular}
\end{center}
\caption{The 3 most interesting itemsets out of the top 10 picked by the MTV algorithm for hours in US presidential elections day. The actual rank given to each itemset by MTV is in column 3 and the rank of its equivalent \emph{strongly closed} itemset is in column 4.}
...
for different geographies\footnote{http://www.google.ca/trends/}.
%are searching for.
It can also be used to compute
trends of user specified queries
over time\footnote{https://support.google.com/trends/answer/87276?hl=en}.
Queries that are named entities according to Google's Knowledge Graph\footnote{http://www.google.com/insidesearch/features/search/knowledge.html} 
are automatically tracked 
and aggregated by category
into Top Charts\footnote{http://www.google.ca/trends/topcharts}~---~
lists of real-world people, places and things ranked in order of search volume 
%during different intervals of time. 
during user specified intervals of time\footnote{https://support.google.com/trends/answer/3076011?hl=en}.
We use the Top Charts of November 2012 
for the people category\footnote{http://www.google.ca/trends/topcharts\#vm=chart\&cid=people\&geo=US\&date=201211}
...
as displayed by Google trends\footnote{http://www.google.ca/trends/explore\#q=Barack\%20Obama\%2C\%20Mitt\%20Romney\&geo=US\&date=11\%2F2012\%201m\&cmpt=q}. 
The two curves in the plot are similar; 
both have peaks at the elections day
and are otherwise flat.
Note that the y-Axis in the figure is scaled\footnote{https://support.google.com/trends/answer/87282?hl=en\&ref\_topic=13975} so that the 
...
the MTV Europe Music Awards on November 11th\footnote{http://en.wikipedia.org/wiki/2012\_MTV\_Europe\_Music\_Awards} 
and the American Music Awards on November 18th\footnote{http://en.wikipedia.org/wiki/American\_Music\_Awards\_of\_2012}. 
...
which are all variations of the itemset: \{\#emawinARTIST, ARTIST, NAME, \#mtvema, i, think, will, be, the, big, winner, tweet, your, pick, at, URL\footnote{http://ema-twittertracker.mtv.com/live/predict.html}\}.
...
how Justin Bieber appeared with his mother Pattie Mallette\footnote{http://hollywoodlife.com/2012/11/18/justin-bieber-brings-mom-to-amas-without-selena-gomez/\#!1/nokia-jenny-mccarthy/}
...
Carly Rae Jepsen also performed and won at the American Music Awards\footnote{http://blog.muchmusic.com/carly-rae-jepsen-wins-at-the-american-music-awards-puts-on-fun-and-flirty-performance/}.
...
in the last 3 months of 2012 is available for download\footnote{http://plg.uwaterloo.ca/$\sim$yaboulna/thesis\_results/twitter\_synopses}. 
% at URL.
