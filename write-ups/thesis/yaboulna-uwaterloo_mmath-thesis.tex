...
other languages. 
Notice that we do not remove function words for reasons that will be
detailed later.

\item
Finally, there is considerable redundancy in frequent
...
that users naturally undergo by sharing and participating in conversations on social media.
Our main contributions are as follows:
\begin{itemize}
\item Proposed the use of variable length N-grams as items to avoid mining uninformative language constructs as itemsets. 
\item Proposed conditions for selecting informative itemsets and reducing redundancy in the mining results.
\item Proposed a formula for ranking itemsets according to novelty.
\end{itemize}
The effectiveness of our methods is shown quantitatively, as the drop of the number of itemsets at each processing step. 
...
We categorize them into algorithms that operate in the item space, 
...
in the sense that an itemset can be expanded by an item regardless
...
to count the support of candidates. 
...
of frequency. For each item, (1) it is output as an itemset,
...
The closure property~\cite{pasquier1999discovering,pasquier1999efficient,zaki2002charm} prunes an itemset
...
which is linked to the former properties. 
...
The Galios connection is a pair of operators, one to map an itemset to transactions in which it appears, 
...
Some algorithms take advantage of the Closed Property to 
...
We will explain the closure property and describe LCM in detail in the section \ref{sec:lcm}. 
...
Related approach to picking out representative itemsets are: 
...
The $\delta$-free~\cite{boulicaut2003free} and 
the $\delta$-covered~\cite{xin2005mining} sets are
examples of conditions motivated by providing a compressed representation 
...
set size'' \cite{liu2012finding}. The \emph{distinct}  condition strengthens the closure condition.
...
are hard clusters. 
Also, the distance measure used in $\delta$-clusters is one minus the Jaccard similarity,
...
Selecting the top-k itemsets according to an interestingness criterion was first proposed by Xin et al.~\cite{xin2006extracting}. 
The major difference between their approach and all of the previous works is that it emphasizes both 
interestingness and redundancy on the selected top-k itemsets, 
...
\item \emph{Bond (or Coherence)}~\cite{omiecinski2003alternative}: The support of an itemset over the number of transactions containing any of its items.
\end{enumerate}
...
Preliminary experimentation with various other measures ruled them out as ineffective 
...
The model could be initialized using itemset frequencies known
...
In the tokenization process, they use CHARM to mine closed itemsets from 
frequent tokens in the document.
They then add the document to the postings lists of each closed itemset
present in the document. 
Queries are tokenized similarly and ranking is done using TF-IDF.
...
and the higher length of documents as opposed to the length of transactions typical to market basket data.
...
They address the problems of overlap in closed patterns,
...
temporally consecutive batches of data using a pyramidal time window. 
Their framework allows for fast execution of temporal queries, and for tracking the evolution of topics. 
However, the choice of itemsets from each batch is based on their utility in compressing the data, 
and thus they use non-negative matrix factorization to extract topics related to a query. 
...
(nicknamed the ``Iran's Twitter Revolution''
...
In a study comparing the use Twitter's search feature and Web search, 
...
or giving higher weights to posts made by informers 
...
In this framework, a set of keywords are tracked and could possibly be all tokens in the dataset.
...
where there are two states and a token is moved to the state of being bursty
...
O'Connor et al.~\cite{o2010tweetmotif} also start from phrases and 
select \emph{significant} ones then cluster them. 
...
%-----------------------------
Besides, the memory requirements of the FP-Growth algorithm suffers from the sparsity
