...
In this paper, we expand on the Linear-time Closed itemset Mining (LCM) algorithm \cite{uno2004lcm}, 
...
we begin our presentation with these properties.
...
%does not result
results in an itemset with greater support.
Notice that they are necessarily at the end of the itemset,
...
\begin{enumerate}
\item An itemset must be extended by all the items that occur in $T_{itemset}$, 
...
\KwIn{$\alpha$: Support ratio} %Dynamic
...
\ForEach{$w \in X.tokens$}{
	...
	\uIf(\tcp*[f]{Support threshold for 1-itemsets}){$|T_{\{w\}}| \geq \alpha * |E|$}{ %\frac{|E|}{E.span}
		LCM($\{w\}, w, T_{\{w\}}$) \;
	}
}
\Return{C}\;
...
\Fn{LCM(s: Current itemset,   $w_{sh}$: Suffix head, \\ \hspace*{1.2in}$T_s$: Transactions (tweets) containing s)}{
	...
	\ForEach{$v \succ w_{sh}$ and $v \notin $ suffix}{
		\If{frequency[$v$] $\geq \alpha * |E|$}{%\frac{|E|}{E.span}
...
coincide with
the presidential debates\footnote{http://www.uspresidentialelectionnews.com/2012-debate-schedule/2012-presidential-debate-schedule/}, and
...
$\alpha$ multiplied by the \emph{epoch}'s volume velocity. 
...
This shows the
We now proceed by discussing how 
we apply the algorithms to streaming data.
...
~\cite{ganti2002mining,bifet2007learning,gilbert2001surfing,zhu2003efficient}, 
...
the oldest data point is removed from the window as the newest data point is added.
The data in the window is processed each time the window slides.
