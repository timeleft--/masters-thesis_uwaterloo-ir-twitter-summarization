...
We use the \emph{distinct} condition to filter out itemsets that are redundant, then we cluster the remaining ones into \emph{strong closed} itemset clusters. Unlike the original LCM algorithm, filtering low confidence itemsets requires keeping mining results in memory to calculate the confidence of newly generated ones. In our experience with the Twitter dataset, it was not necessary to keep all previous itemsets in memory, but rather the last 1000. Thus, the runtime and memory requirement is still bounded. 


\subsection{Implementation}

The clustering of itemsets can be implemented efficiently using techniques similar to the ones proposed by Bayardo et al. \cite{bayardo2007scaling}. The main ideas are to limit the comparisons to a few candidates, and to terminate the comparison early if the similarity threshold will not be met. In our case the postings lists are longer than the itemsets, so we generate candidates for comparison by calculating similarity between itemsets. When calculating the similarity between two postings lists, we can terminate early if the difference exceeds the maximum difference permissible to achieve a similarity of $\kappa$, which can be easily derived from equation \ref{eq:conf}. 
...
Algorithm  \ref{algo:alliance}  shows a possible implementation. For each itemset, $s_i$, we find the itemsets produced before it and overlapping with it in one or more item. Out of those candidates, $s_c$, we find the one that maximizes $conf(s_c \rightarrow s_i)$ such that the confidence exceeds $\kappa$.
...
\SetAlgoLined
\LinesNumbered
\KwIn{$\kappa$: Minimum confidence threshold} % closed strength} %$\theta$: Minimum cosine similarity, \\
