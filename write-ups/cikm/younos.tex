...
\For{$i \gets 2$ to $|\mathcal{C}|$}{
	...
	\ForEach{$s_c \in C$}{
		\uIf{$s_c \cap s_i = s_c$}{
			$\Delta \gets (1-\kappa) |T_{s_c}|$ \tcp*{Distinctiveness}
			$\delta \gets |T_{s_c}| - |T_{s_i}|$\tcp*{Candidate is a subset}
		}	
		\Else{
			$\Delta \gets (1 - (1-\kappa)) |T_{s_i}|$ \tcp*{Similarity}
			$\delta \gets$ difference($T_{s_i},T_{s_c \cup s_i},\Delta$) \tcp*{Stops early}
		}
