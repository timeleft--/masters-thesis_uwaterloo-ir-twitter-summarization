...
\For{$i \gets 2$ to $|\mathcal{C}|$}{
	...
	\uIf{$\frac{|T_{s_i}|}{|T_{s_p}|} < \kappa$}{continue\tcp*{Not distinct}}
	%$s_m
	$maxConf \gets 0$ \tcp*{Best candidate's score}
	\ForEach{$s_c \in C$}{
		$\Delta \gets (1 - (1-\kappa)) |T_{s_c}|$ \tcp*{Similarity}
	...
	%\If{$minDiff < \infty$}{
	\If{$maxConf > 0$}{
		$\mathcal{R}[s_i] = \mathcal{R}[s_m]$ \tcp*{Cluster $s_i$ with $s_m$}
		$\mathcal{R}[s_m].itemset \gets \mathcal{S}[s_m].itemset \cup s_i \cup s_m$\;
		$\mathcal{R}[s_m].postingsList \gets \mathcal{S}[s_m].postingsList \cup s_i.postingsList \cup s_m.postingsList$\;
	}
}
\Return{$\mathcal{S}$}\;
