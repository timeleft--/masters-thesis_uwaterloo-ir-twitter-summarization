...
unigramsStruct = LOAD 'hbase://tokenPos' USING yaboulna.pig.HBaseStorage('d:121221', '-versions 1000 -eq the') AS (token: chararray, days: {(day: chararray, vers: {(id: long, posArr:{(pos: int)})})});
...
unigrams = FOREACH unigramsStruct3 GENERATE token, day, id, FLATTEN(posArr);
SPLIT unigrams INTO 
  unigramsP0 IF pos==0,
  unigramsP1 IF pos==1,
  unigramsP2 IF pos==2,
