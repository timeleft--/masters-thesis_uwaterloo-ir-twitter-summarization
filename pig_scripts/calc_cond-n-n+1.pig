...
-- Unsorted: Orderby fails because some stupid input path doesn't exist exception from the partitioner
...
prob2g1Avg = FOREACH prob2g1Grps GENERATE FLATTEN(group) as (ngram2, ngram1), 
	AVG($1.condProb) as avgCondProb, AVG($1.unigramProb) as avgUnigramProb, AVG($1.volume) AS avgVolume,
	MIN($1.condProb) as minCondProb, MIN($1.unigramProb) as minUnigramProb, MIN($1.volume) AS minVolume,
	MAX($1.condProb) as maxCondProb, MAX($1.unigramProb) as maxUnigramProb, MAX($1.volume) AS maxVolume; 

STORE prob2g1Avg INTO '$root/prob/ngram2g1Avg' USING PigStorage('\t');

prob2g1Days = GROUP prob2g1 BY (ngram2, ngram1, date);
prob2g1Daily = FOREACH prob2g1Days GENERATE FLATTEN(group) as (ngram2, ngram1, date), 
	AVG($1.condProb) as avgCondProb, AVG($1.unigramProb) as avgUnigramProb, AVG($1.volume) AS avgVolume,
	MIN($1.condProb) as minCondProb, MIN($1.unigramProb) as minUnigramProb, MIN($1.volume) AS minVolume,
	MAX($1.condProb) as maxCondProb, MAX($1.unigramProb) as maxUnigramProb, MAX($1.volume) AS maxVolume; 
