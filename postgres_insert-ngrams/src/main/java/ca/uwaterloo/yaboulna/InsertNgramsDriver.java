...
// File[] inputFiles =
// FileUtils.listFiles(FileUtils.toFile(inputRoot.toUri().toURL()),noHiddenOrLogs,noHiddenOrLogs).toArray(new File[0]);
        ...
        if (!complete) {
          String report =
              (j + " (" + jobArr[j].getJobName() + "): map "
                  ...
                  + jobArr[j].mapProgress() +
                  " reduce " +
                  // StringUtils.formatPercent(jobArr[j].reduceProgress(), 0) +
                  +jobArr[j].reduceProgress() +
                  " - Tracking: " + jobArr[j]
                  .getTrackingURL());
          LOG.info(report);
        }
