...
echo "${psql} \"COPY compgrams${len}_${day} FROM '${root}/occ_extended${len}/${day}.csv'; \
 CREATE TABLE cnt_${epoch}${len}_${day} AS select CAST( ${len} as int2) as ngramlen, CAST('{' || ngram || '}' as text[]) as ngramarr, CAST (${day} as int4) as date, cast( floor(timemillis/3600000) * 3600000 as int8) as epochstartmillis, cast(count(*) as int4) as cnt from compgrams${len}_${day} group by ngram,floor(timemillis/3600000) * 3600000; \
