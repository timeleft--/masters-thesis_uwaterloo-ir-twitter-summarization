...
do
    ...
    echo "${psql} 'CREATE TABLE cnt_${epoch} (ngram text, date int4, epochStartMillis int8, cnt int4);'" #, pkey serial Primary key);'"
#    for len in 1 2  #{1..71}
    len=$3
    while [ $len -le $4 ] 
    do
	# echo "hdfs dfs -cat ${root}/cnt_${epoch}/ngrams1/part* | ${psql} 'COPY cnt_${epoch} (ngram, date, epochstartmillis, cnt) FROM STDIN;'"
	echo "${psql} 'CREATE TABLE cnt_${epoch}${len} (ngram text, date int4, epochStartMillis int8, cnt int4);'"
...
#	echo "${psql} 'CREATE INDEX cnt_${epoch}${len}_ngramLen ON cnt_${epoch}${len}(ngramLen);'"
    ...
    echo "${psql} 'CREATE INDEX volume_${epoch}${len}_date ON volume_${epoch}${len}(date);'"
	echo ""
	len=`expr $len + 1`
    done
done
