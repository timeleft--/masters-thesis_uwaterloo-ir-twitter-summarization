...
#echo "${psql} 'CREATE INDEX compgrams${len}_date ON compgrams${len}_{$day}(date);'"
#echo "${psql} 'CREATE INDEX compgrams${len}_ngramLen ON compgrams${len}_${day}(ngramLen);'"

echo "${psql} \"CREATE TABLE cnt_${epoch}${len}_${day} as select  ${len} as ngramLen, regexp_split_to_array(ngram,'+') as ngramarr, ${day} as date, floor(timemillis/3600000) * 3600000 as epochstartmillis, count(*) as cnt from compgrams${len}_${day} group by floor(timemillis/3600000),ngram;\""

echo "${psql} 'CREATE TABLE volume_${epoch}${len}_${day} as select  ${len} as ngramLen, epochstartmillis, sum(cnt) as totalcnt from cnt_${epoch}${len}_${day} group by epochstartmillis;'"
